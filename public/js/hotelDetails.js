...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const proceedToPaymentButton = document.getElementById('proceed-to-payment-btn');
    ...
    reserveButtons.forEach(button => {
        button.addEventListener('click', () => {
            ...
            addToCart(roomId, hotelId, roomPrice, checkIn, checkOut, nights);
        });
    });
...
function addToCart(roomId, hotelId, roomPrice, checkIn, checkOut, nights) {
    fetch('/cart/add', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        if (data.success) {
            if (confirm('Room added to cart successfully! Do you want to view your cart?')) {
    ...
    .catch((error) => {
        ...
        alert('An error occurred while adding the room to the cart. Please check the console for more details and try again.');
    });
}
